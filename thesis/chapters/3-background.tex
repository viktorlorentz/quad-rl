...
\begin{itemize}
  \item \textbf{Deterministic policy:}  
    \[
      a_t = \pi_{\theta}(s_t),
    \]
    where \(\pi_{\theta}\) is a function (e.g., a neural network) mapping state \(s_t\) to action \(a_t\).
...
\end{align}
In practice, there are different approaches to approximate the policy gradient \(\nabla_{\theta}J(\theta)\). In the next section, we will focus on the \gls{ppo} algorithm, which is a popular on‐policy method.
