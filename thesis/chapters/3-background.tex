...
The reward function \(r_t(s_t,a_t)\) provides immediate feedback to the agent and usually depends on the current state \(s_t\), action taken by the policy \(a_t\) and the next state \(s_{t+1}\). The return \(R_t\) at time step \(t\) is defined as the sum of discounted rewards:
\begin{equation}
