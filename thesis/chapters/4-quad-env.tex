...
\todo{maybe further domain randomization}
...
\begin{figure}
  ...
  \caption[Reward Components]{
    ...
    (b) A heatmap showing the interplay between payload position and velocity reward. The reward is highest when the payload is close to the target and has very low velocity. A maximum velocity of 1.5m/s is encouraged.
  }
...
\end{equation}
Figure~\ref{fig:reward_details} shows the interplay of the payload position and velocity reward. Building a funnel on the position with highest reward at the target position and low velocity.
