...
\begin{table*}[!b]
  \centering
  \caption[Summary of \gls{rl} approaches]{Summary of recent \gls{rl} approaches for Crazyflie control. In the table, $\mathbf{p}$ denotes position, $\mathbf{e}$ the position errors, $\mathbf{v}$ velocity, $\mathbf{R}$ a rotation matrix (orientation), and $\boldsymbol{\omega}$ angular velocity. For details refer to the original paper. We group PWM and motor thrust commands as \gls{srt}.}
