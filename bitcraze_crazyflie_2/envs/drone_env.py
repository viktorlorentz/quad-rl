...
class DroneEnv(MujocoEnv):
    ...
    def compute_reward(
        ...
        angular_vel_penalty = (
            rc["angular_velocity"] * angular_velocity_penalty
        )
        ...
        # Goal bonus
        if distance < 0.1:
            goal_bonus = rc["goal_bonus"] * np.exp(
                -(distance**2) / 0.05**2
            )
            reward += goal_bonus
